...
// First we should decrease each element with 1
    ...
    for(int i=0;i<n;i++){
        ...
        // once again because we dont know the index of it and also 2 is in two places then there is other question that where we gonna be store its occurence. 
        // So that's why we are using that element which index == current element (after decreasing each element by 1), in this if 2 comes first time 
        // we just go to 2 index and add n to that element and after if we got another 2 then again we go to that increment element and add 7 to it.
        // Easy peasy.
