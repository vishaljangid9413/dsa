...
void recurrsive_view(Node *root, vector<int> &ans, vector<int> &level_vector, int position, int level){
    if(root == NULL){
        return;
    }

    // if current node's level is smaller than the stored node's level 
    // than replace it with smaller one because we have to find the top view
    if(level_vector[position] > level){
        level_vector[position] = level;
        ans[position] = root->data;
    }

    // but if current nodes level is higher than the stored nodes level 
    // so do not replace them they gives us the bottom level nodes
    // because we have to find the bottom view
    // if(level_vector[position] < level){
    //     level_vector[position] = level;
    //     ans[position] = root->data;
    // }

    recurrsive_view(root->left, ans, level_vector, position - 1, level + 1);
    recurrsive_view(root->right, ans, level_vector, position + 1, level + 1);
}
...
int main(){
    ...
    // if you want to find the top view store maximum value
    int default_level = INT_MAX;
    // and if you want to find the bottom view store minimum value 
    // int default_level = INT_MIN;
    
    //holds the answer
    vector<int>ans(right - left + 1);
    vector<int>level_vector(right - left + 1, default_level);
    int level = 0;
    int position = -1 * left;
    recurrsive_view(root, ans, level_vector, position, level);
