...
2. **Feasibility Check (`possible` function)**:
...
int main(){
    ...
    // for example if have total 15 bananas and three hours to eat, monkey have to eat minimum 5 banans to finish all in given time 
    long long sum = 0;
    int maxi = 0;
    for (int i = 0; i < n; i++){
        sum += arr[i];
    ...
    int end = maxi; // Upper bound for speed (the largest pile)
